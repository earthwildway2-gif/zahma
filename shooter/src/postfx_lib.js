...
/* ================= our own composer setup: bloom + film grain + color grade + vignette ================= */
const GradeShader = {
  uniforms: { tDiffuse: { value: null }, time: { value: 0 }, vig: { value: 0.42 }, grain: { value: 0.028 }, warm: { value: 0.06 }, sat: { value: 1.08 }, contrast: { value: 1.06 } },
  vertexShader: 'varying vec2 vUv; void main(){ vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position,1.0); }',
  fragmentShader: `
    uniform sampler2D tDiffuse; uniform float time, vig, grain, warm, sat, contrast; varying vec2 vUv;
    float hash(vec2 p){ return fract(sin(dot(p, vec2(41.7, 289.1))) * 43758.5453); }
    void main(){
      vec3 c = texture2D(tDiffuse, vUv).rgb;
      c *= vec3(1.0 + warm * 0.5, 1.0 + warm * 0.15, 1.0 - warm * 0.35);           // subtle warm/teal split (orange highlights, cool shadow)
      float lum = dot(c, vec3(0.299, 0.587, 0.114));
      c = mix(vec3(lum), c, sat);
      c = (c - 0.5) * contrast + 0.5;
      vec2 d = vUv - 0.5; float vg = 1.0 - dot(d, d) * vig * 2.2; c *= clamp(vg, 0.0, 1.0);
      c += (hash(vUv * vec2(1920.0, 1080.0) + time) - 0.5) * grain;
      gl_FragColor = vec4(clamp(c, 0.0, 1.0), 1.0);
    }`
};
let composer = null, bloomPass = null, gradePass = null, fxaaPass = null, usingComposer = false;
function setupComposer() {
  if (!renderer || typeof THREE.EffectComposer === 'undefined') return;
  try {
    composer = new THREE.EffectComposer(renderer);
    composer.addPass(new THREE.RenderPass(scene, camera));
    bloomPass = new THREE.UnrealBloomPass(new THREE.Vector2(256, 256), 0.55, 0.55, 0.82); composer.addPass(bloomPass);
    if (typeof THREE.FXAAShader !== 'undefined') { fxaaPass = new THREE.ShaderPass(THREE.FXAAShader); composer.addPass(fxaaPass); }
    gradePass = new THREE.ShaderPass(GradeShader); gradePass.renderToScreen = true; composer.addPass(gradePass);
    usingComposer = true;
  } catch (e) { usingComposer = false; }
}
function composerResize(w, h) {
  if (!composer) return; const pr = renderer.getPixelRatio(); composer.setSize(w, h);
  if (bloomPass) bloomPass.setSize(w, h);
  if (fxaaPass) fxaaPass.material.uniforms.resolution.value.set(1 / (w * pr), 1 / (h * pr));
}
function renderFrame(t) {
  if (usingComposer) { if (gradePass) gradePass.uniforms.time.value = t * 0.001; composer.render(); }
