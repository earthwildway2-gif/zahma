...
<title>الليلة الطويلة — لعبة تصويب</title>
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Cairo:wght@500;700;900&display=swap">
<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
<style>
:root{--bg:#05070d;--panel:rgba(10,13,22,.86);--fg:#f2ead8;--dim:#98a0b3;--accent:#ff8a2b;--red:#ff3b3b;--ok:#4fd28a;box-sizing:border-box;
  padding-top:env(safe-area-inset-top,0px);padding-bottom:env(safe-area-inset-bottom,0px)}
html{height:100%;scroll-padding-top:env(safe-area-inset-top,0px)}
html,body{margin:0;background:var(--bg);color:var(--fg);overflow:hidden}
body{height:100%;position:relative;font-family:Cairo,"Segoe UI",Tahoma,Arial,sans-serif;-webkit-user-select:none;user-select:none;touch-action:none;-webkit-tap-highlight-color:transparent}
canvas#c{position:absolute;inset:0;width:100%;height:100%;display:block}
.hidden{display:none!important}
#vig{position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse at center,rgba(0,0,0,0) 55%,rgba(0,0,0,.55) 100%)}
#dmg{position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse at center,rgba(255,0,0,0) 30%,rgba(190,0,0,.75) 100%);opacity:0}
...
#cross i{position:absolute;background:rgba(255,255,255,.9);box-shadow:0 0 2px #000}
#cross .h{height:2px;width:9px}#cross .v{width:2px;height:9px}
#hit{position:absolute;left:50%;top:50%;width:26px;height:26px;margin:-13px 0 0 -13px;pointer-events:none;opacity:0}
#hit:before,#hit:after{content:"";position:absolute;left:12px;top:-3px;width:2px;height:32px;background:#fff;transform:rotate(45deg)}
#hit:after{transform:rotate(-45deg)}
#hit.head:before,#hit.head:after{background:var(--accent)}
#hud{position:absolute;left:0;right:0;top:0;bottom:0;pointer-events:none}
#hp{position:absolute;left:12px;top:10px;width:150px}
#hp .bar{height:10px;background:rgba(0,0,0,.55);border:1px solid rgba(255,255,255,.25);border-radius:6px;overflow:hidden}
#hp .bar i{display:block;height:100%;width:100%;background:linear-gradient(90deg,#d13,#ff6a3d)}
#hp small{font-size:11px;color:var(--dim)}
#ammo{position:absolute;left:12px;bottom:118px;text-align:left;direction:ltr;text-shadow:0 1px 4px #000}
#ammo b{font-size:30px;font-weight:900}#ammo span{font-size:15px;color:var(--dim)}#ammo div{font-size:12px;color:var(--accent);direction:rtl;text-align:left}
#obj{position:absolute;top:10px;left:50%;transform:translateX(-50%);max-width:58%;background:var(--panel);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:5px 12px;font-weight:700;font-size:13px;text-align:center}
#feed{position:absolute;right:10px;top:46px;text-align:right;font-size:13px;font-weight:700;text-shadow:0 1px 3px #000;direction:rtl}
#feed div{opacity:1;transition:opacity .6s}
#sub{position:absolute;left:8%;right:8%;bottom:16%;text-align:center;font-size:18px;font-weight:700;line-height:1.6;text-shadow:0 2px 6px #000,0 0 3px #000}
#sub b{color:var(--accent);margin-inline-end:6px}
#bars i{position:absolute;left:0;right:0;height:0;background:#000;transition:height .8s;z-index:6}
#bars i:first-child{top:0}#bars i:last-child{bottom:0}
body.cine #bars i{height:12%}
#msg{position:absolute;left:50%;top:32%;transform:translateX(-50%);font-size:26px;font-weight:900;text-align:center;text-shadow:0 2px 10px #000;opacity:0;transition:opacity .4s;pointer-events:none;white-space:nowrap}
#msg.on{opacity:1}
/* touch */
#stick{position:absolute;left:0;bottom:0;width:46%;height:52%;touch-action:none}
#knob{position:absolute;width:92px;height:92px;border-radius:50%;border:2px solid rgba(255,255,255,.35);background:rgba(255,255,255,.08);display:none}
#knob i{position:absolute;left:31px;top:31px;width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.4)}
#look{position:absolute;right:0;top:0;width:54%;height:100%;touch-action:none}
.btn{position:absolute;border-radius:50%;border:2px solid rgba(255,255,255,.35);background:rgba(10,13,22,.5);color:var(--fg);font:900 15px Cairo,Arial;display:flex;align-items:center;justify-content:center;touch-action:none;padding:0;z-index:5}
.btn.on{background:rgba(255,138,43,.7)}
#bF{right:22px;bottom:34px;width:92px;height:92px;font-size:18px;background:rgba(255,60,60,.35);border-color:#ff5a5a}
#bR{right:128px;bottom:26px;width:56px;height:56px}
#bW{right:24px;bottom:142px;width:56px;height:56px}
#bS{left:22px;bottom:36%;width:52px;height:52px}
body.desk .touch{display:none}
body.touchmode #cross{opacity:.85}
.screen{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:24px;z-index:20;background:linear-gradient(180deg,rgba(5,7,13,.6),rgba(5,7,13,.97) 75%)}
.screen h1{font-size:54px;font-weight:900;margin:0;color:var(--accent);line-height:1.15;text-shadow:0 0 30px rgba(255,138,43,.4)}
.screen h2{font-size:30px;font-weight:900;margin:0 0 8px}
.screen p{max-width:460px;line-height:1.85;color:var(--dim);font-size:16px;margin:10px 0}
.screen .sub2{font-size:18px;color:var(--fg);font-weight:700}
.go{margin-top:16px;font:900 22px Cairo,Arial;background:var(--accent);color:#1a0d02;border:0;border-radius:14px;padding:13px 44px}
.go:active{transform:scale(.97)}
#stats{color:var(--fg);font-weight:700;line-height:1.9}
</style>
</head>
<body class="desk">
<canvas id="c"></canvas>
<div id="vig"></div><div id="dmg"></div><div id="flash"></div>
<div id="cross"><i class="h" style="left:-14px;top:-1px"></i><i class="h" style="left:5px;top:-1px"></i><i class="v" style="left:-1px;top:-14px"></i><i class="v" style="left:-1px;top:5px"></i></div>
<div id="hit"></div>
<div id="hud" class="hidden">
  <div id="hp"><small>الصحة</small><div class="bar"><i></i></div></div>
  <div id="obj"></div>
  <div id="feed"></div>
  <div id="ammo"><b id="am">12</b> <span id="ar">/ 60</span><div id="wn">مسدس</div></div>
</div>
<div id="sub"></div>
<div id="msg"></div>
<div id="bars"><i></i><i></i></div>
<div id="stick" class="touch hidden"><div id="knob"><i></i></div></div>
<div id="look" class="touch hidden"></div>
<button class="btn touch hidden" id="bF">نار</button>
<button class="btn touch hidden" id="bR">تعمير</button>
<button class="btn touch hidden" id="bW">سلاح</button>
<button class="btn touch hidden" id="bS">جري</button>
<div id="title" class="screen">
  <h1>الليلة الطويلة</h1>
  <div class="sub2">الفصل الثاني — الشنطة</div>
  <p>اللي وصّلته امبارح مكانش هدوم. شحتة اتمسك عشان شاف اللي جوّه، وسيد ماشي لوحده داخل مخزن الحاج صلاح في الهرم الساعة تلاتة الفجر.</p>
  <p id="how">حرّك بالعصا الشمال، والفت بالسحب على اليمين، وزرار "نار" للضرب. على الكمبيوتر: WASD والماوس، وR لتعمير السلاح، و1 2 3 لتغيير السلاح.</p>
  <button class="go" id="start">ابدأ</button>
  <p id="nogl" class="hidden"></p>
</div>
<div id="end" class="screen hidden"><h2 id="endT"></h2><p id="endP"></p><div id="stats"></div><button class="go" id="again">العب تاني</button></div>
<script>
(() => {
'use strict';
...
scene.add(yawObj); yawObj.add(pitchObj); pitchObj.add(camera); scene.add(camera.parent);
...
{ SP.pos = new Float32Array(SP.n * 3); SP.col = new Float32Array(SP.n * 3); for (let i = 0; i < SP.n; i++) { SP.vel.push(new V3()); SP.life.push(0); SP.pos[i * 3 + 1] = -999; }
  ...
  SP.pts = new THREE.Points(SP.geo, new THREE.PointsMaterial({ size: 0.09, vertexColors: true, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, fog: false })); SP.pts.frustumCulled = false; effG.add(SP.pts); }
...
// shell casings
const casings = []; { const geo = new THREE.BoxGeometry(0.03, 0.03, 0.09), mat = new THREE.MeshPhongMaterial({ color: 0xd8a830, shininess: 80 });
...
{ for (let i = 0; i < RAIN.n; i++) { RAIN.pos[i * 3] = rand(-18, 18); RAIN.pos[i * 3 + 1] = rand(0, 16); RAIN.pos[i * 3 + 2] = rand(-18, 18); }
  RAIN.geo = new THREE.BufferGeometry(); RAIN.geo.setAttribute('position', new THREE.BufferAttribute(RAIN.pos, 3)); const rp = new THREE.Points(RAIN.geo, new THREE.PointsMaterial({ color: 0x9db4d8, size: 0.06, transparent: true, opacity: 0.45, depthWrite: false })); rp.frustumCulled = false; effG.add(rp); RAIN.pts = rp; }
...
window.__g = { P, W, enemies, S, kills: () => kills, get state() { return state; }, explode, spawnEnemies, begin, fire, storyUpdate, camera, sparks, FX, items, barrels, gate, rollDoor };
